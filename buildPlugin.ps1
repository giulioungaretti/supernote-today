...
function Build-AndroidApk {
    ...
    try {
        ...
        if (Test-Path $gradlewPath) {
            ...
            $gradleCommand = "`"$gradlewPath`" buildCustomApkDebug"
            $process = Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $gradleCommand -WorkingDirectory $androidDir -Wait -PassThru -NoNewWindow
...
function Main {
    ...
    if (-not $bundleSuccess) {
        throw 'React Native bundling failed'
    }
    ...
    if ($shouldBuildNative) {
        ...
        if ($buildSuccess) {
            ...
            if (-not $apkCopied) {
                throw 'Native APK built, but it could not be copied into the plugin package'
            }
        } else {
            throw 'Native Android code is present, but the Gradle APK build failed'
        }
        ...
        $generatedConfig = Get-Content $buildGeneratedConfigFile -Raw | ConvertFrom-Json
        $nativePackagePath = Join-Path $buildGeneratedDir 'app.npk'
        if (-not $generatedConfig.nativeCodePackage -or -not (Test-Path $nativePackagePath)) {
            throw 'Native Android code is present, but build/generated is missing app.npk or nativeCodePackage'
        }
    }
    else {
    ...
    if (-not $zipResult) {
        throw 'Packaging failed, unable to create zip file'
    }
    ...
    if (Test-Path $finalSnplgPath) {
        ...
        throw 'Failed to rename the package to .snplg'
    }
}
...
try {
    ...
    Write-ColorOutput "Plugin build failed: $($_.Exception.Message)" 'Red'
    exit 1
}
