    ...
    if [[ "$should_build_native" -eq 0 ]]; then
        ...
        if build_android_apk "$project_root" "$gen_cfg"; then
            copy_apk_and_update_config "$project_root" "$gen_dir" "$gen_cfg" || {
                write_color_output "Native APK built, but it could not be copied into the plugin package" "Red"
                return 1
            }
        else
            write_color_output "Native Android code is present, but the Gradle APK build failed" "Red"
            return 1
        fi

        if [[ ! -f "$gen_dir/app.npk" ]]; then
            write_color_output "Native Android code is present, but build/generated/app.npk is missing" "Red"
            return 1
        fi
    else
